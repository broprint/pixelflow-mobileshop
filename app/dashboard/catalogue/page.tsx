...
function DeviceVisual({ brand, model, imageUrl }: { brand: string; model: string; imageUrl?: string | null }) {
  ...
  return (
    <div className="relative flex h-32 w-24 items-center justify-center rounded-[1.5rem] border-[5px] border-slate-800 bg-gradient-to-br from-slate-100 via-white to-violet-100 shadow-inner">
      ...
      <div className="text-center"><p className="text-xl font-black text-violet-600">{brand.slice(0, 1)}</p><p className="mt-1 px-1 text-[9px] font-semibold leading-tight text-slate-500">{model}</p></div>
    </div>
  );
}
...
export default async function CataloguePage({ searchParams }: { searchParams: Promise<{ added?: string; error?: string; q?: string; brand?: string; storage?: string }> }) {
  ...
  const [{ data: products }, { data: variantImages }] = await Promise.all([
  ...
  const imageByVariant = new Map((variantImages ?? []).map((image) => [image.variant_id, image.image_url]));
      ...
      <div className="mx-auto max-w-7xl p-5 md:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-slate-500">PixelFlow maintains the device catalogue once. Your shop chooses the exact storage and colour variants it wants to sell, then controls its own price, stock and warranty.</p>
          </div>
        ...
        <section className="mt-7 rounded-3xl border border-violet-100 bg-white p-5 shadow-sm">
          <form method="get" className="grid gap-3 lg:grid-cols-[1fr_220px_220px_auto]">
            ...
            <select name="brand" defaultValue={brandFilter} className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-violet-400"><option value="all">All brands</option>{brands.map((brand) => <option key={brand} value={brand}>{brand}</option>)}</select>
            <select name="storage" defaultValue={storageFilter} className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-violet-400"><option value="all">All storage</option>{storages.map((storage) => <option key={String(storage)} value={String(storage)}>{formatStorage(Number(storage))}</option>)}</select>
            ...
            <article key={product.id} className="overflow-hidden rounded-3xl border border-violet-100 bg-white shadow-sm">
              ...
              <div className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">
                {product.product_variants.map((variant) => {
                  ...
                  const variantImage = imageByVariant.get(variant.id) ?? product.image_url;
                  const hasColourImage = imageByVariant.has(variant.id);

                  return (
                    <div key={variant.id} className="rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-violet-50/50 p-5">
                      <div className="mb-4 flex h-36 items-center justify-center rounded-2xl border border-slate-100 bg-white p-3">
                        {variantImage ? <img src={variantImage} alt={`${product.brand} ${product.model} ${variant.color ?? ""}`} className="h-full w-full object-contain" /> : <span className="text-xs text-slate-400">No product image</span>}
                      </div>
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div><p className="text-lg font-black">{formatStorage(variant.storage_gb)}</p><p className="mt-1 font-semibold text-slate-700">{variant.color || "Standard colour"}</p><p className="mt-1 text-[11px] font-medium text-slate-400">SKU: {variant.sku}</p></div>
                        <div className="flex flex-col items-end gap-1">{alreadyAdded ? <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">In inventory</span> : null}{hasColourImage ? <span className="rounded-full bg-violet-50 px-2.5 py-1 text-[10px] font-bold text-violet-600">Colour image</span> : null}</div>
                      </div>
