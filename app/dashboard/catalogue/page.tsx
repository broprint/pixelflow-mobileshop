...
function DeviceVisual({ brand, model, imageUrl }: { brand: string; model: string; imageUrl?: string | null }) {
  if (imageUrl) {
    return <img src={imageUrl} alt={`${brand} ${model}`} className="h-32 w-24 rounded-2xl object-contain" />;
  }

  return (
    <div className="relative flex h-32 w-24 items-center justify-center rounded-[1.5rem] border-[5px] border-slate-800 bg-gradient-to-br from-slate-100 via-white to-violet-100 shadow-inner">
      <div className="absolute left-1/2 top-2 h-1.5 w-9 -translate-x-1/2 rounded-full bg-slate-800" />
      <div className="text-center">
        <p className="text-xl font-black text-violet-600">{brand.slice(0, 1)}</p>
        <p className="mt-1 px-1 text-[9px] font-semibold leading-tight text-slate-500">{model}</p>
      </div>
    </div>
  );
}
...
export default async function CataloguePage({
  ...
  searchParams: Promise<{ added?: string; error?: string; q?: string; brand?: string; storage?: string }>;
}) {
  ...
  const filteredProducts = allProducts
    .map((product) => {
      ...
      const variants = (product.product_variants ?? []).filter((variant) => {
        if (storageFilter !== "all" && String(variant.storage_gb) !== storageFilter) return false;
        return true;
      });
      ...
      <div className="mx-auto max-w-7xl p-5 md:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-slate-500">
              PixelFlow maintains the device catalogue once. Your shop chooses the exact storage and colour variants it wants to sell, then controls its own price, stock and warranty.
            </p>
          </div>
        ...
        <section className="mt-7 rounded-3xl border border-violet-100 bg-white p-5 shadow-sm">
          <form method="get" className="grid gap-3 lg:grid-cols-[1fr_220px_220px_auto]">
            <input name="q" defaultValue={params.q ?? ""} placeholder="Search Apple, Samsung, model..." className="rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100" />
            <select name="brand" defaultValue={brandFilter} className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-violet-400">
              <option value="all">All brands</option>
              {brands.map((brand) => <option key={brand} value={brand}>{brand}</option>)}
            </select>
            <select name="storage" defaultValue={storageFilter} className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-violet-400">
              <option value="all">All storage</option>
              {storages.map((storage) => <option key={String(storage)} value={String(storage)}>{formatStorage(Number(storage))}</option>)}
            </select>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-2 text-sm text-slate-500">
            <span>{filteredProducts.length} models • {visibleVariantCount} variants shown</span>
            {(query || brandFilter !== "all" || storageFilter !== "all") ? <Link href="/dashboard/catalogue" className="font-semibold text-violet-600">Clear filters</Link> : null}
          </div>
        </section>
        ...
        <section className="mt-6 grid gap-6">
            ...
            <article key={product.id} className="overflow-hidden rounded-3xl border border-violet-100 bg-white shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-5 border-b border-slate-100 p-6">
                <div className="flex items-center gap-5">
                  <DeviceVisual brand={product.brand} model={product.model} imageUrl={product.image_url} />
                  <div>
                    <p className="text-sm font-bold text-violet-600">{product.brand}</p>
                    <h2 className="mt-1 text-2xl font-black">{product.model}</h2>
                    <p className="mt-2 text-sm capitalize text-slate-500">{product.category} • {product.product_variants.length} variants</p>
                  </div>
                </div>
                  ...
                  return (
                    <div key={variant.id} className="rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-violet-50/50 p-5">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <p className="text-lg font-black">{formatStorage(variant.storage_gb)}</p>
                          <p className="mt-1 font-semibold text-slate-700">{variant.color || "Standard colour"}</p>
                          <p className="mt-1 text-[11px] font-medium text-slate-400">SKU: {variant.sku}</p>
                        </div>
                        {alreadyAdded ? <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">In inventory</span> : null}
                      </div>

                      <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
                        {variant.ram_gb ? <span className="rounded-full bg-white px-2.5 py-1 shadow-sm">{variant.ram_gb}GB RAM</span> : null}
                        {variant.network ? <span className="rounded-full bg-white px-2.5 py-1 shadow-sm">{variant.network}</span> : null}
                      ...
                      {profile.role === "shop_admin" && !alreadyAdded && variant.is_active ? (
                        <form action={addVariantToShop} className="mt-5 grid gap-3 border-t border-slate-100 pt-5">
                          ...
                          <label className="text-xs font-bold text-slate-500">Warranty
                            <input name="warranty_text" placeholder="e.g. 1 Year" className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100" />
                          </label>
                          <button type="submit" className="rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-violet-700">Add to My Shop</button>
                        </form>
                      ) : null}
          ...
          {!filteredProducts.length ? (
            <div className="rounded-3xl border border-dashed border-violet-200 bg-white p-10 text-center">
              <h2 className="text-xl font-black">No catalogue matches</h2>
              <p className="mt-2 text-slate-500">Try a different brand, storage size or search term.</p>
              <Link href="/dashboard/catalogue" className="mt-4 inline-block font-bold text-violet-600">Show all products</Link>
            </div>
          ) : null}
        </section>
      </div>
