      ...
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-slate-400">Maintain a master model image plus optional colour-specific images for exact variants. Variant images override the master image in the retailer catalogue.</p>
          </div>
        ...
        {params.saved ? <div className="mt-6 rounded-2xl border border-emerald-800 bg-emerald-950/50 px-5 py-3 text-sm font-semibold text-emerald-300">Image saved successfully.</div> : null}
              ...
              <article key={product.id} className="overflow-hidden rounded-3xl border border-slate-800 bg-slate-900 shadow-lg shadow-black/10">
                <div className="grid gap-6 p-5 lg:grid-cols-[250px_1fr]">
                  <div>
                    ...
                    <form action={uploadCatalogueImage} encType="multipart/form-data" className="mt-4 grid gap-2 border-t border-slate-800 pt-4">
                      ...
                      <button className="rounded-xl bg-cyan-400 px-4 py-2.5 text-sm font-black text-slate-950 hover:bg-cyan-300">{product.image_url ? "Replace Master Image" : "Upload Master Image"}</button>
                    </form>
                      ...
                      {productVariants.map((variant) => {
                        ...
                        return (
                          <div key={variant.id} className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
                            ...
                            <form action={uploadVariantImage} encType="multipart/form-data" className="mt-3 grid gap-2">
                              ...
                              <button className="rounded-lg bg-violet-600 px-3 py-2 text-xs font-bold hover:bg-violet-500">{variantImage ? "Replace colour image" : "Upload colour image"}</button>
                            </form>
