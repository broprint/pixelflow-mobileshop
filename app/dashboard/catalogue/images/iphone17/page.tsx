...
import { importIPhone17ColourImages } from "./actions";

const COLORS = ["Black", "White", "Mist Blue", "Sage", "Lavender"];

export default async function IPhone17ImagesPage({
  searchParams,
}: {
    ...
    <main className="min-h-screen bg-slate-950 px-5 py-8 text-white md:px-8">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-6 rounded-3xl border border-slate-800 bg-slate-900 p-6">
          ...
          <h1 className="mt-2 text-3xl font-black">iPhone 17 colour images</h1>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-400">
            One clean image is stored for each colour and reused automatically across all iPhone 17 storage variants of that colour.
          </p>
          ...
          {params.saved ? (
            <div className="mt-5 rounded-2xl border border-emerald-800 bg-emerald-950/50 px-4 py-3 text-sm font-semibold text-emerald-300">
              All five iPhone 17 colour images were imported into PixelFlow storage.
            </div>
          ) : null}
            ...
            {COLORS.map((color) => {
              ...
              return (
                <div key={color} className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3">
                  <div className="flex h-40 items-center justify-center rounded-xl bg-slate-900 p-3">
                    {imageUrl ? (
                      <img src={imageUrl} alt={`Apple iPhone 17 ${color}`} className="h-full w-full object-contain" />
                    ) : (
