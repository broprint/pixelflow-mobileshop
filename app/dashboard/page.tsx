...
export default async function DashboardPage() {
  ...
  let shops: { id: string; name: string; slug: string; status: string }[] = [];
  if (profile.role === "super_admin") {
    const result = await supabase.from("shops").select("id,name,slug,status").order("created_at");
    shops = result.data ?? [];
  } else if (profile.shop_id) {
    const result = await supabase.from("shops").select("id,name,slug,status").eq("id", profile.shop_id);
    shops = result.data ?? [];
  }
      ...
      <div className="mx-auto max-w-6xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-bold">{profile.role === "super_admin" ? "Super Admin" : "Shop Admin"} Dashboard</h1>
        ...
        <section className="mt-10 grid gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
            <p className="text-slate-400">Role</p><p className="mt-2 text-xl font-bold">{profile.role}</p>
          </div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
            <p className="text-slate-400">Visible shops</p><p className="mt-2 text-xl font-bold">{shops.length}</p>
          </div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
            <p className="text-slate-400">Tenant isolation</p><p className="mt-2 text-xl font-bold text-emerald-400">RLS enabled</p>
          </div>
        </section>

        <section className="mt-8 rounded-3xl border border-slate-800 bg-slate-900 p-6">
          <h2 className="text-xl font-bold">{profile.role === "super_admin" ? "All Shops" : "Your Shop"}</h2>
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            {shops.map((shop) => (
              <div key={shop.id} className="rounded-2xl border border-slate-800 bg-slate-950 p-5">
                <div className="flex items-center justify-between gap-3">
                  <h3 className="font-bold">{shop.name}</h3>
                  <span className="rounded-full bg-emerald-950 px-3 py-1 text-xs text-emerald-300">{shop.status}</span>
                </div>
